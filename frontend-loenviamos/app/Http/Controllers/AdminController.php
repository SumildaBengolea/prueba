...
class AdminController extends Controller
{
    ...
    public function restaurants()
    {
        ...
        $metodosPagos = DB::table('metodosPagos')->where('is_active','1')->get();
        $count = $restaurants->total();
        
        return view('admin.restaurants', array(
            'restaurants' => $restaurants,
            'count' => $count,
            'metodos'=>$metodosPagos,
        ));
    }
    ...
    public function searchRestaurants(Request $request)
    {
        ...
        $count = $restaurants->total();
        if($count == 0 ){
          $restaurants = Restaurant::Select()->paginate(20);
        }
        $metodosPagos = DB::table('metodosPagos')->where('is_active','1')->get();   
    ...
    public function saveNewRestaurant(Request $request)
    {
        ...
        $restaurant->separador_mil_moneda = $request->separador_mil_moneda;
        $restaurant->simbolo_moneda = $request->simbolo_moneda;
        $restaurant->decimal_moneda = $request->moneda_decimal;
        $restaurant->codigo_moneda = $request->restaurant_moneda;
        ...
        try {
            $restaurant->save();
            sleep(1);
            $idn = DB::table('restaurants')->select('id')->where('name', $restaurant->name)->first();
            $this->updateMetodoPago($request, $idn->id);
    ...
    public function getEditRestaurant($id)
    {
        ...
        $metodosPagos = DB::table('metodosPagos')->where('is_active','1')->get();
        $count = restaurantPagos::where('id_restaurant', $id)->count();
        
        if ($count == 0) {
          restaurantPagos::create(['id_restaurant' => $id, 'pasarelas' => '']);
          sleep(5);
        }

        $metodosActivos = restaurantPagos::where('id_restaurant', $id)->first();

        $pasarelas = explode(',',$metodosActivos->pasarelas);
        ...
        return view('admin.editRestaurant', array(
            ...
            'metodos'=> $metodosPagos,
            'metodosActivos' => $pasarelas,
        ));
    }

    public function updateMetodoPago($req, $ids = null) {
    $metodos = [];
    
    $pasarelas = DB::table('metodosPagos')->where('is_active','1')->get();
    
    foreach($pasarelas as $row){
      if ($req->has($row->pasarela)) {
        array_push($metodos, $row->pasarela);
      }
    } 
 
     $metodosp = implode(',', $metodos);
 
    if ($req->has('id')) {
      $id = $req->id;
      $data = ['id_restaurant' => $id, 'pasarelas' => $metodosp];
      $met = restaurantPagos::find($id);
      $met->fill($data)->save();
    } else {
      $data = ['id_restaurant' => $ids, 'pasarelas' => $metodosp];
      $add = restaurantPagos::create($data);
    }
  }
